...
Expected usage in Experiment B (conceptually):
    ...
    saliency = adapter.predict(image_rgb, actions=["sit", "grasp", ...])
...
# OOAL model
from models.ooal import Net as OOALNet

# Affordance name lists (as in test.py)
from data.agd20k_ego import SEEN_AFF, UNSEEN_AFF
...
def _cli():
    ...
    sal = adapter.predict(rgb, args.actions)
