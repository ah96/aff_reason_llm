...
def iter_image_ids(ade_ann_dir: str, limit: Optional[int] = None, split: str = "train") -> List[str]:
    ...
    """
    full_path = ade_ann_dir + split + "/"
    # print("\nfull_path: ", full_path)
    files = sorted([f for f in os.listdir(full_path) if f.endswith(".png") and f.startswith(f"ADE_{split}_")])
...
def main():
    ...
    ap.add_argument("--ade_aff_dir", required=True, help="Path to ADE-Affordance files (relationship.txt + exco.json).")
    ap.add_argument("--llms", required=True, help="Path to configs/llms.json")
    ap.add_argument("--split", default="train", choices=["train", "validation"], help="ADE split prefix: train/validation")
    ...
    if not image_ids:
        raise RuntimeError(f"No ADE annotation PNGs found in {args.ade_ann_dir + args.split} for split={args.split}")
    # print("\nimage_ids: ", image_ids)
    ...
    for image_id in tqdm(image_ids, desc="Images"):
        print("\nimage_id: ", image_id)
        ann_png = os.path.join(args.ade_ann_dir + args.split, image_id + ".png")
        print("\nann_png: ", ann_png)
        rel_txt = os.path.join(args.ade_aff_dir, image_id + "_relationship.txt")
        print("\nrel_txt: ", rel_txt)
        exco_js = os.path.join(args.ade_aff_dir, image_id + "_exco.json")
        print("\nexco_js: ", exco_js)
